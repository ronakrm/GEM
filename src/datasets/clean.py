###############
## copied from https://github.com/algowatchpenn/GerryFair
###############
...
def clean_dataset(dataset, attributes, centered):
    ...
    # binarize any continous-y sensitives
    for s in sens_names:
        if not isinstance(X[s][0], int):
            # we have a sens attribute not yet binarized
            column = X[s]
            med = np.median(column)
            X[s] = (X[s] > med)*1

    # split sens features from train features
    X_prime = X[sens_names]
    X = X.drop(sens_names, axis=1)
