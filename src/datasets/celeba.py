...
class CelebA(data.Dataset):
	...
	"""
	def __init__(self, root, train, target, spurious, transform=None, n_samples=None,
		...
		try:
			self.part_data = pd.read_csv(os.path.join(root, self.partition_file), 
												delimiter=" ", names=['id', 'partition'])
			# self.part_data.rename(columns=['id', 'partition'], inplace=True)
		...
		self.target_labels = self.sel_attrs[self.target].to_numpy() 

		# +1 for skipping imagename
		# self.spurious = [i+1 for i in self.spurious]
		self.spurious_labels = self.sel_attrs[self.spurious].to_numpy() 
	...
	def __getitem__(self, index):
		...
		return img, torch.Tensor([int(target_label), int(spurious_label[0])])
